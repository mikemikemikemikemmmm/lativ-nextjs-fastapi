...
def get_nav_by_route(db: SessionDepend, nav_route: str):
    stmt = text(
        """
        SELECT
            n.id,
...
def get_series(
    ...
    stmt = text(
        ...
        FROM nav n 
        INNER JOIN category c ON c.nav_id = n.id AND n.route = :nav_route AND c.route = :category_route
        INNER JOIN sub_category sc ON sc.category_id = c.id AND sc.route = :sub_category_route
