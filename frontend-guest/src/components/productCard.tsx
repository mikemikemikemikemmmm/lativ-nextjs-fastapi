import type { ProductCardRead } from "@/types";
...
export const ProductCard = (props: { pc: ProductCardRead }) => {
    ...
    return <div className="flex flex-col text-center">
        <Link to={`/product/${pc.id}`} className="block mb-2" >
            <Image height={1200} width={800} src={getImgUrl(pc.img_url)} alt={pc.name} />
        </Link>

        <div className="flext justify-center items-center">
            {
                pc.sub_products.map(sp => <Link to={`/product/${pc.id}?sub_product_id=${sp.id}`} className="inline-block my-2 mx-1" style={{ width: 16, height: 16 }} key={sp.id}>
        ...
        <div className="mt-auto">
            NT$ {
                pc.sub_products.length > 0
                    ? pc.sub_products.reduce((prev, curr) => curr.price < prev.price ? curr : prev).price
                    : '-'
            }</div>
    </div>
}