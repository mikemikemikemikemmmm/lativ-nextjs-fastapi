...
class SeriesModel(BaseSQLModel):
    ...
    sub_category_id: Mapped[int] = mapped_column(ForeignKey("sub_category.id", ondelete="RESTRICT"))
    sub_category: Mapped["SubCategoryModel"] = relationship(back_populates="series")

    products: Mapped["ProductModel"] = relationship(back_populates="series", passive_deletes=True)
